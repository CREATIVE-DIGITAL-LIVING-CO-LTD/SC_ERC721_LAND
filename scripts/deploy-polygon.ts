...
const { PROXY_REGISTRY_POLYGON, ADDRESS_MAINNET } = process.env
async function main() {
    ...
    const contract = await ANIV721Land.deploy(PROXY_REGISTRY_POLYGON!, 30000)
    try {
        await contract.deployed()
        console.log("Token address:", contract.address)
        const newItemId = await contract.mint(ADDRESS_MAINNET!, 4331)
